import 'package:flutter/material.dart';

class AppConstants {
  static const String appName = 'اداره امور';

  // Colors
  static const Color primaryColor = Color(0xff1B047C);
  static const Color backgroundColor = Colors.white;
  static const Color shadowColor = Colors.black;

  // Dimensions
  static const double defaultPadding = 16.0;
  static const double headerPadding = 20.0;
  static const double headerImageHeight = 100.0;

  // API URLs

  // Asset paths
  static const String logoPath = 'assets/logo.png';
  static const String profilePath = 'assets/profile.png';
  static const String aopPath = 'assets/aop.png';

  // Language flags
  static const String persianFlagPath = 'assets/flags/persian.png';
  static const String englishFlagPath = 'assets/flags/english.png';
  static const String pashtoFlagPath = 'assets/flags/pashto.png';

  // Route names
  static const String homeRoute = '/';
  static const String languageRoute = '/language';
  static const String webViewRoute = '/webview';
  static const String pashtoRoute = '/pashto';
  static const String persianRoute = '/persian';
  static const String englishRoute = '/english';
  static const String settingsRoute = '/settings';
  static const String serviceRoute = '/service';
  static const String feedbackRoute = '/feedback';

  // Styles
  static const TextStyle headerTitleStyle = TextStyle(
    fontSize: 24,
    fontWeight: FontWeight.bold,
    color: Colors.white,
  );

  static const TextStyle appBarTitleStyle = TextStyle(
    fontSize: 20,
    fontWeight: FontWeight.bold,
    color: Colors.white,
  );

  static final BoxDecoration headerDecoration = BoxDecoration(
    color: primaryColor,
    boxShadow: [
      BoxShadow(
        color: shadowColor.withAlpha(51), // 0.2 opacity = 51/255
        blurRadius: 8,
        offset: const Offset(0, 4),
      ),
    ],
  );

  // URLs
  static const Map<String, String> aopUrls = {
    'persian': 'https://aop.gov.af/dr',
    'pashto': 'https://aop.gov.af/pa',
    'english': 'https://aop.gov.af/en',
  };

  static const Map<String, String> aopVideosUrls = {
    'persian': 'https://aop.gov.af/dr/videos',
    'pashto': 'https://aop.gov.af/dr/pictures',
    'english': 'https://aop.gov.af/en/videos',
  };

  static const Map<String, String> aopMinistryUrls = {
    'persian': 'https://aop.gov.af/dr/government/ministries',
    'pashto': 'https://aop.gov.af/pa/government/ministries',
    'english': 'https://aop.gov.af/en/government/ministries',
  };

  static const Map<String, String> passportUrls = {
    'persian': 'https://passport.moi.gov.af/da',
    'pashto': 'https://passport.moi.gov.af/pa',
    'english': 'https://passport.moi.gov.af/pa',
  };

  static const Map<String, String> aopFormUrls = {
    'persian': 'https://aop.gov.af/dr/forms',
    'pashto': 'https://aop.gov.af/pa/forms',
    'english': 'https://aop.gov.af/en/forms',
  };

  // Multilingual welcome messages
  static const String welcomeAppMessagePashto = 'د ادارې ایپ ته ښه راغلاست.';
  static const String welcomeAppMessagePersian = 'به اپلیکیشن اداره امور خوش آمدید';
  static const String welcomeAppMessageEnglish = 'Welcome to the AOP App';

  // Persian Text
  static const Map<String, String> persianText = {
    'videos': 'ویدیوها',
    'welcome': 'خوش آمدید',
    'headerTitle': 'ریاست عمومی اداره امور',
    'aopWebsite': 'وبسایت اداره امور',
    'agencyList': 'لیست وزارت خانه ها',
    'publicServices': 'خدمات عامه',
    'passportServices': 'خدمات پاسپورت',
    'changeLanguage': 'تغییر زبان',
    'comments': 'نظریات',
    'contactUs': 'تماس با ما',
    'settings': 'تنظیمات',
    'home': 'خانه',
    'websiteNav': 'وبسایت',
    'contactNav': 'تماس',
    'dayToDay': 'روز به روز',
    'dayToDayDescription': 'وسایل روزمره مانند هوا، نرخ تبادله، اخبار، قبله و وقت نماز به زودی در این بخش قرار می‌گیرد.',
    'newsNav': 'اخبار',
    'jobOpportunities': 'فرصت های شغلی',
    'searchJobs': 'جستجو در فرصت های شغلی...',
    'noJobsFound': 'هیچ فرصت شغلی یافت نشد',
    'noJobsFoundDescription': 'در حال حاضر فرصت شغلی موجود نیست',
    'newJobs': 'بست‌های جدید',
    'expiringJobs': 'بست‌های در حال انقضا',
    'expiredJobs': 'بست‌های منقضی شده',
    // Qibla Compass
    'qiblaCompass': 'قطب نمای قبله',
    'qiblaDirection': 'جهت قبله',
    'calibrateCompass': 'کالیبراسیون قطب نما',
    'calibrationInstructions': 'برای کالیبراسیون دقیق، دستگاه را به شکل هشت حرکت دهید',
    'locationPermissionRequired': 'دسترسی به موقعیت مکانی مورد نیاز است',
    'enableLocationServices': 'خدمات موقعیت مکانی را فعال کنید',
    'compassNotAvailable': 'قطب نما در دسترس نیست',
    'qiblaAngle': 'زاویه قبله',
    'yourLocation': 'موقعیت شما',
    'distanceToKaaba': 'فاصله تا کعبه',
    'kilometers': 'کیلومتر',
    'noNewJobsFound': 'هیچ بست جدیدی یافت نشد',
    'noNewJobsFoundDescription': 'در حال حاضر بست جدید موجود نیست',
    'noExpiringJobsFound': 'هیچ بست در حال انقضا یافت نشد',
    'noExpiringJobsFoundDescription': 'در حال حاضر بست در حال انقضا موجود نیست',
    'noExpiredJobsFound': 'هیچ بست منقضی شده یافت نشد',
    'noExpiredJobsFoundDescription': 'در حال حاضر بست منقضی شده موجود نیست',
    'jobOpportunity': 'فرصت شغلی',
    'jobInformation': 'اطلاعات بست',
    'jobDescription': 'شرح بست',
    'noDescriptionAvailable': 'شرح بست موجود نیست',
    'jobType': 'نوع بست',
    'status': 'وضعیت',
    'announcementDate': 'تاریخ اعلان',
    'deadline': 'مهلت',
    'contractDuration': 'مدت قرارداد',
    'applyNow': 'اکنون درخواست دهید',
    'applyViaEmail': 'درخواست از طریق ایمیل',
    'applyOnline': 'درخواست آنلاین',
    'emailClientOpened': 'برنامه ایمیل با موفقیت باز شد',
    'browserOpened': 'مرورگر با موفقیت باز شد',
    'linkOpened': 'لینک با موفقیت باز شد',
    'emailClientError': 'نمی‌توان برنامه ایمیل را باز کرد. لطفاً بررسی کنید که برنامه ایمیل نصب شده باشد.',
    'browserError': 'نمی‌توان مرورگر را باز کرد. لطفاً اتصال اینترنت خود را بررسی کنید.',
    'linkError': 'نمی‌توان لینک را باز کرد.',
    'active': 'فعال',
    'expired': 'منقضی شده',
    'generalError': 'خطایی رخ داده است',
    'noInternetConnection': 'اتصال به اینترنت برقرار نیست',
    'sharedFromApp': 'از اپلیکیشن اداره امور به اشتراک گذاشته شده',
    'noResults': 'نتیجه‌ای یافت نشد',
    'feedbackGuide': 'لطفاً نظریات/شکایات خود را با ما شریک سازید',
    'whatsapp': 'واتساپ',
    'email': 'ایمیل',
    'contactForm': 'فورم تماس',
    'langGuide': 'لطفاً زبان خود را انتخاب نمایید',
    'loading': 'در حال بارگذاری...',
    'changingLanguage': 'در حال تغییر زبان...',
    // News Screen Texts
    'newsTitle': 'اخبار',
    'newsTabsAll': 'همه',
    'newsTabsLatest': 'آخرین اخبار',
    'newsTabsSocial': 'اجتماعی',
    'newsTabsEconomic': 'اقتصادی',
    'newsTabsPolitical': 'سیاسی',
    'newsTabsCultural': 'فرهنگی',
    'searchNewsHint': 'جستجو در اخبار...',
    'clearSearch': 'پاک کردن',
    'searchLabel': 'جستجو برای:',
    'emptySearchResult': 'نتیجه‌ای برای "{query}" یافت نشد',
    'emptySearchSuggestion': 'لطفا با کلمات کلیدی دیگری جستجو کنید',
    'clearSearchButton': 'پاک کردن جستجو',
    'savedNews': 'اخبار ذخیره شده',
    'noSavedNews': 'هیچ خبری ذخیره نشده است',
    'newsRemoved': 'خبر حذف شد',
    'undo': 'بازگشت',
    'browseNews': 'مشاهده اخبار',
    'newsSaved': 'خبر ذخیره شد',
    'newsUnsaved': 'خبر از لیست ذخیره‌ها حذف شد',
    'clearAllSavedNews': 'حذف همه اخبار ذخیره شده',
    'clearAllSavedNewsConfirm': 'آیا مطمئن هستید که می‌خواهید همه اخبار ذخیره شده را حذف کنید؟',
    'cancel': 'انصراف',
    'clear': 'حذف',
    'allNewsRemoved': 'همه اخبار ذخیره شده حذف شدند',
    'save': 'ذخیره',
    'changeTextSize': 'تغییر اندازه متن',
    'comingSoon': 'به زودی',
    'fullscreenComingSoon': 'نمایش تمام صفحه به زودی',
    'imageGallery': 'گالری تصاویر',
    'imageCount': '{count} تصویر',
    'noImages': 'تصویری وجود ندارد',
    'additionalInfo': 'اطلاعات بیشتر',
    'viewCount': '{count} بازدید',
    'noTitle': 'بدون عنوان',
    'viewFullNews': 'مشاهده کامل خبر در برنامه ما',
    'newsItem': 'مورد خبری',
    'errorLoadingNews': 'خطا در بارگیری خبر',
    // Ministry Screen Texts
    'ministries': 'وزارت‌خانه‌ها',
    'searchMinistries': 'جستجو در وزارت‌خانه‌ها...',
    'noMinistries': 'هیچ وزارت‌خانه‌ای وجود ندارد',
    'ministryComingSoon': 'به زودی وزارت‌خانه‌های جدید اضافه خواهند شد',
    'ministryLoadError': 'خطا در بارگیری لیست وزارت‌خانه‌ها',
    'tryAgain': 'تلاش مجدد',
    'viewDetails': 'مشاهده جزئیات',
    'ministry': 'وزارت‌خانه',
    'aboutMinistry': 'درباره وزارت‌خانه',
    'contactInfo': 'اطلاعات تماس',
    'viewOfficialWebsite': 'مشاهده وب‌سایت رسمی',
    'phone': 'تلفن',
    'website': 'وب‌سایت',
    'share': 'اشتراک‌گذاری',
    'shareError': 'خطا در اشتراک‌گذاری',
    'noWebsite': 'وب‌سایتی موجود نیست',
    'back': 'بازگشت',
    // Independent Directorates Text
    'independentDirectorates': 'ریاست‌های مستقل',
    'searchDirectorates': 'جستجو در ریاست‌های مستقل...',
    'noDirectorates': 'هیچ ریاست مستقلی وجود ندارد',
    'directorateComingSoon': 'به زودی ریاست‌های مستقل جدید اضافه خواهند شد',
    'directorateLoadError': 'خطا در بارگیری لیست ریاست‌های مستقل',
    'directorate': 'ریاست مستقل',
    'aboutDirectorate': 'درباره ریاست مستقل',
    // Provinces Text
    'provinces': 'ولایات',
    'searchProvinces': 'جستجو در ولایات...',
    'noProvinces': 'هیچ ولایتی وجود ندارد',
    'provinceComingSoon': 'به زودی ولایات جدید اضافه خواهند شد',
    'provinceLoadError': 'خطا در بارگیری لیست ولایات',
    'province': 'ولایت',
    'aboutProvince': 'درباره ولایت',
    // Error messages
    'cannotOpenWebsite': 'متاسفانه نمی‌توان وب‌سایت را باز کرد: {url}',
    'cannotMakeCall': 'متاسفانه نمی‌توان تماس برقرار کرد: {phone}',
    // API Error Messages
    'cannotLoadData': 'خطا در بارگیری اطلاعات',
    // Filter related texts
    'filter': 'فیلتر',
    'filterByDepartment': 'فیلتر بر اساس وزارت‌خانه',
    'allDepartments': 'همه وزارت‌خانه‌ها',
    'activeJobs': 'بست‌های فعال',
    'clearFilter': 'پاک کردن فیلتر',
    'applyFilter': 'اعمال فیلتر',
    'noJobsInDepartment': 'هیچ بستی در این وزارت‌خانه یافت نشد',
    'noConnection': 'اتصال به اینترنت برقرار نیست',
    'timeout': 'زمان درخواست به پایان رسید',
    'serverError': 'خطا در سرور رخ داده است',
    'prayerTimesLoadError': 'اوقات نماز بارگیری نشد.',
    'retry': 'تلاش مجدد',
    'backToHome': 'بازگشت به صفحه اصلی',
    'offline': 'شما آفلاین هستید',
    'checkConnection': 'لطفا اتصال اینترنت خود را بررسی کنید',
    // Contract Types
    'permanent': 'دائمی',
    'temporary': 'موقت',
    'contract': 'قراردادی',
    'partTime': 'پاره وقت',
    'fullTime': 'تمام وقت',
    // Solar Hijri Months
    'hamal': 'حمل',
    'sawr': 'ثور',
    'jawza': 'جوزا',
    'saratan': 'سرطان',
    'asad': 'اسد',
    'sunbula': 'سنبله',
    'mizan': 'میزان',
    'aqrab': 'عقرب',
    'qaws': 'قوس',
    'jadi': 'جدی',
    'dalw': 'دلو',
    'hut': 'حوت',
    // Weather
    'weather': 'آب و هوا',
    'exchangeRate': 'نرخ تبادله',
    'newsTabsSports': 'ورزش',
    'newsTabsGeneral': 'عمومی',
    'newsTabsOthers': 'دیگر',
    'prayerTime': 'وقت نماز',
    'dateConverter': 'تبدیل تاریخ',
    'dateConverterDescription': 'تبدیل میان تاریخ‌های میلادی، هجری شمسی و قمری.',
    'date': 'تاریخ',
    'convert': 'تبدیل',
    'miladi': 'میلادی',
    'gregorian': 'میلادی',
    'shamsi': 'شمسی',
    'qamari': 'قمری',
    'prayerTimes': 'اوقات نماز',
    'shamsiDate': 'تاریخ شمسی',
    'gregorianDate': 'تاریخ میلادی',
    'selectShamsiDate': 'انتخاب تاریخ شمسی',
    'showTimes': 'نمایش اوقات',
    'validDateError': 'یک تاریخ معتبر به شکل YYYY/MM/DD وارد کنید.',
    'validShamsiDateError': 'یک تاریخ شمسی معتبر به شکل YYYY/MM/DD وارد کنید.',
    'qamariConversionNote': 'تبدیل قمری از تقویم مدنی استفاده می‌کند؛ رؤیت رسمی ماه ممکن است یک روز تفاوت داشته باشد.',
    'estimatedKabulTimes': 'اوقات تخمینی کابل — برای استفاده از اوقات رسمی، ردیف‌های جدول اوقات را اضافه کنید.',
    'fajr': 'فجر',
    'dhuhr': 'ظهر',
    'asr': 'عصر',
    'maghrib': 'مغرب',
    'isha': 'عشا',
    'dailyPrayerTimes': 'اوقات روزانه',
    'dateConverterShortDescription': 'شمسی / میلادی / قمری',
    'currentPrayer': 'نماز فعلی',
    'nextPrayer': 'نماز بعدی',
    'kabulTime': 'وقت کابل',
    'currentWeather': 'آب و هوای فعلی',
    'temperature': 'دما',
    'feelsLike': 'احساس می‌شود',
    'humidity': 'رطوبت',
    'windSpeed': 'سرعت باد',
    'pressure': 'فشار هوا',
    'visibility': 'دید',
    'uvIndex': 'شاخص UV',
    'airQuality': 'کیفیت هوا',
    'weatherCondition': 'وضعیت آب و هوا',
    'sunrise': 'طلوع آفتاب',
    'sunset': 'غروب آفتاب',
    'locationPermissionDenied': 'دسترسی به موقعیت مکانی رد شد',
    'weatherLoadError': 'خطا در بارگیری اطلاعات آب و هوا',
    'refreshWeather': 'به‌روزرسانی آب و هوا',
    'defaultLocation': 'کابل، افغانستان',
    'celsius': '°C',
    'fahrenheit': '°F',
    'kmh': 'کیلومتر بر ساعت',
    'mph': 'مایل بر ساعت',
    'hpa': 'هکتوپاسکال',
    'km': 'کیلومتر',
    'percent': '%',
  };

  // Pashto Text
  static const Map<String, String> pashtoText = {
    'videos': 'ویډیوګانې',
    'welcome': 'ښه راغلاست',
    'headerTitle': 'د چارو ادارې لوی ریاست',
    'aopWebsite': 'د چارو ادارې ویب پاڼه',
    'agencyList': 'د وزارتونو لیست',
    'publicServices': 'عامه خدمتونه',
    'passportServices': 'د پاسپورت خدمتونه',
    'changeLanguage': 'د ژبې بدلول',
    'comments': 'نظرونه',
    'contactUs': 'له موږ سره اړیکه',
    'settings': 'تنظیمات',
    'home': 'کور',
    'websiteNav': 'ویب پاڼه',
    'contactNav': 'اړیکه',
    'dayToDay': 'ورځ په ورځ',
    'dayToDayDescription': 'د هوا، تبادلي نرخ، خبرونو، قبلې او لمونځ وخت په شان ورځنۍ وسایل به ژر پدې برخه کې ښودل کېږي.',
    'newsNav': 'خبرونه',
    'jobOpportunities': 'د کار فرصتونه',
    'searchJobs': 'د کار فرصتونو کې پلټنه...',
    'noJobsFound': 'د کار هیڅ فرصت ونه موندل شو',
    'noJobsFoundDescription': 'اوس مهال د کار فرصت شتون نلري',
    'newJobs': 'نوي دندې',
    'expiringJobs': 'د ختمېدو په حال کې دندې',
    'expiredJobs': 'ختم شوي دندې',
    'noNewJobsFound': 'هیڅ نوې دنده ونه موندل شوه',
    'noNewJobsFoundDescription': 'اوس مهال نوې دنده شتون نلري',
    'noExpiringJobsFound': 'د ختمېدو په حال کې هیڅ دنده ونه موندل شوه',
    'noExpiringJobsFoundDescription': 'اوس مهال د ختمېدو په حال کې دنده شتون نلري',
    'noExpiredJobsFound': 'هیڅ ختم شوې دنده ونه موندل شوه',
    'noExpiredJobsFoundDescription': 'اوس مهال ختم شوې دنده شتون نلري',
    // Qibla Compass
    'qiblaCompass': 'د قبلې قطب نما',
    'qiblaDirection': 'د قبلې لور',
    'calibrateCompass': 'د قطب نما کالیبریشن',
    'calibrationInstructions': 'د سمې کالیبریشن لپاره، خپل وسیله د اتو په شکل حرکت ورکړئ',
    'locationPermissionRequired': 'د ځای د موندنې اجازه ته اړتیا ده',
    'enableLocationServices': 'د ځای د موندنې خدمات فعال کړئ',
    'compassNotAvailable': 'قطب نما شتون نلري',
    'qiblaAngle': 'د قبلې زاویه',
    'yourLocation': 'ستاسو ځای',
    'distanceToKaaba': 'کعبې ته واټن',
    'kilometers': 'کیلومتره',
    'jobOpportunity': 'د کار فرصت',
    'jobInformation': 'د بست معلومات',
    'jobDescription': 'د بست تشریح',
    'noDescriptionAvailable': 'د بست تشریح شتون نلري',
    'jobType': 'د بست ډول',
    'status': 'حالت',
    'announcementDate': 'د اعلان نیټه',
    'deadline': 'وخت',
    'contractDuration': 'د تړون موده',
    'applyNow': 'اوس غوښتنه وکړئ',
    'applyViaEmail': 'د بریښنالیک له لارې غوښتنه',
    'applyOnline': 'آنلاین غوښتنه',
    'emailClientOpened': 'د بریښنالیک اپلیکیشن په بریالیتوب سره پرانیستل شو',
    'browserOpened': 'براوزر په بریالیتوب سره پرانیستل شو',
    'linkOpened': 'لینک په بریالیتوب سره پرانیستل شو',
    'emailClientError': 'د بریښنالیک اپلیکیشن نه شي پرانیستل کیدای. مهرباني وکړئ ډاډ ترلاسه کړئ چې د بریښنالیک اپلیکیشن نصب شوی دی.',
    'browserError': 'براوزر نه شي پرانیستل کیدای. مهرباني وکړئ خپل انټرنیټ اتصال وګورئ.',
    'linkError': 'لینک نه شي پرانیستل کیدای.',
    'active': 'فعال',
    'expired': 'ختم شوی',
    'generalError': 'ستونزه رامنځته شوې',
    'noInternetConnection': 'انټرنیټ سره اړیکه نشته',
    'sharedFromApp': 'د اداره امور اپلیکیشن څخه شریک شوی',
    'noResults': 'پایلې ونه موندل شوې',
    'feedbackGuide': 'مهرباني وکړئ خپل نظرونه/شکایتونه له موږ سره شریک کړئ',
    'whatsapp': 'واټساپ',
    'email': 'بریښنالیک',
    'contactForm': 'د اړیکې فورم',
    'langGuide': 'مهرباني وکړئ خپله ژبه غوره کړئ',
    'loading': 'بارېږي...',
    'changingLanguage': 'ژبه بدلېږي...',
    // News Screen Texts
    'newsTitle': 'خبرونه',
    'newsTabsAll': 'ټول',
    'newsTabsLatest': 'وروستي خبرونه',
    'newsTabsSocial': 'ټولنیز',
    'newsTabsEconomic': 'اقتصادي',
    'newsTabsPolitical': 'سیاسي',
    'newsTabsCultural': 'کلتوري',
    'searchNewsHint': 'په خبرونو کې پلټنه...',
    'clearSearch': 'پاکول',
    'searchLabel': 'لټون د:',
    'emptySearchResult': 'د "{query}" لپاره پایله ونه موندل شوه',
    'emptySearchSuggestion': 'مهرباني وکړئ د نورو کلیدي کلمو سره پلټنه وکړئ',
    'clearSearchButton': 'لټون پاک کړئ',
    'savedNews': 'خوندي شوي خبرونه',
    'noSavedNews': 'هیڅ خبر خوندي شوی نه دی',
    'newsRemoved': 'خبر لرې شو',
    'undo': 'بیرته راوستل',
    'browseNews': 'خبرونه وګورئ',
    'newsSaved': 'خبر خوندي شو',
    'newsUnsaved': 'خبر د خوندي شویو له لیست څخه لرې شو',
    'clearAllSavedNews': 'ټول خوندي شوي خبرونه لرې کړئ',
    'clearAllSavedNewsConfirm': 'آیا تاسو ډاډه یاست چې غواړئ ټول خوندي شوي خبرونه لرې کړئ؟',
    'cancel': 'لغوه کول',
    'clear': 'لرې کول',
    'allNewsRemoved': 'ټول خوندي شوي خبرونه لرې شول',
    'save': 'خوندي کول',
    'changeTextSize': 'د متن اندازه بدلول',
    'comingSoon': 'ژر راځي',
    'fullscreenComingSoon': 'بشپړ سکرین به ژر راشي',
    'imageGallery': 'د انځورونو ګالری',
    'imageCount': '{count} انځورونه',
    'noImages': 'هیڅ انځور شتون نلري',
    'additionalInfo': 'اضافي معلومات',
    'viewCount': '{count} لیدنې',
    'noTitle': 'بې سرلیکه',
    'viewFullNews': 'په موږ اپلیکیشن کې بشپړ خبر وګورئ',
    'newsItem': 'خبري توکی',
    'errorLoadingNews': 'د خبر په لوډولو کې ستونزه',
    // Ministry Screen Texts
    'ministries': 'وزارتونه',
    'searchMinistries': 'په وزارتونو کې پلټنه...',
    'noMinistries': 'هیڅ وزارت شتون نلري',
    'ministryComingSoon': 'ژر به نوي وزارتونه اضافه شي',
    'ministryLoadError': 'د وزارتونو د لیست په بارولو کې ستونزه',
    'tryAgain': 'بیا هڅه وکړئ',
    'viewDetails': 'جزئیات وګورئ',
    'ministry': 'وزارت',
    'aboutMinistry': 'د وزارت په اړه',
    'contactInfo': 'د اړیکې معلومات',
    'viewOfficialWebsite': 'رسمي ویب پاڼه وګورئ',
    'phone': 'تلیفون',
    'website': 'ویب پاڼه',
    'share': 'شریکول',
    'shareError': 'په شریکولو کې ستونزه',
    'noWebsite': 'هیڅ ویب پاڼه شتون نلري',
    'back': 'شاته',
    // Independent Directorates Text
    'independentDirectorates': 'خپلواک ریاستونه',
    'searchDirectorates': 'په خپلواک ریاستونو کې پلټنه...',
    'noDirectorates': 'هیڅ خپلواک ریاست شتون نلري',
    'directorateComingSoon': 'ژر به نوي خپلواک ریاستونه اضافه شي',
    'directorateLoadError': 'د خپلواک ریاستونو د لیست په بارولو کې ستونزه',
    'directorate': 'خپلواک ریاست',
    'aboutDirectorate': 'د خپلواک ریاست په اړه',
    // Provinces Text
    'provinces': 'ولایتونه',
    'searchProvinces': 'په ولایتونو کې پلټنه...',
    'noProvinces': 'هیڅ ولایت شتون نلري',
    'provinceComingSoon': 'ژر به نوي ولایتونه اضافه شي',
    'provinceLoadError': 'د ولایتونو د لیست په بارولو کې ستونزه',
    'province': 'ولایت',
    'aboutProvince': 'د ولایت په اړه',
    // Error messages
    'cannotOpenWebsite': 'په خواشینۍ سره ویب پاڼه نشو خلاصولی: {url}',
    'cannotMakeCall': 'په خواشینۍ سره تلیفون نشو کولی: {phone}',
    // API Error Messages
    'cannotLoadData': 'د معلوماتو په بارولو کې ستونزه',
    // Filter related texts
    'filter': 'فلټر',
    'filterByDepartment': 'د وزارت له مخې فلټر',
    'allDepartments': 'ټول وزارتونه',
    'activeJobs': 'فعال دندې',
    'clearFilter': 'فلټر پاک کړئ',
    'applyFilter': 'فلټر پلي کړئ',
    'noJobsInDepartment': 'په دې وزارت کې هیڅ دنده ونه موندل شوه',
    'noConnection': 'انټرنیټ سره اړیکه نشته',
    'timeout': 'د غوښتنې وخت پای ته ورسېد',
    'serverError': 'په سرور کې ستونزه شتون لري',
    'prayerTimesLoadError': 'د لمانځه وختونه پورته نه شول.',
    'retry': 'بیا هڅه وکړئ',
    'backToHome': 'اصلي پاڼې ته ستنېدل',
    'offline': 'تاسو آفلاین یاست',
    'checkConnection': 'مهرباني وکړئ خپله انټرنیټ اړیکه وګورئ',
    // Contract Types
    'permanent': 'دایمي',
    'temporary': 'لنډمهاله',
    'contract': 'تړونیز',
    'partTime': 'د یوې برخې وخت',
    'fullTime': 'بشپړ وخت',
    // Solar Hijri Months
    'hamal': 'وری',
    'sawr': 'غویی',
    'jawza': 'غبرګولی',
    'saratan': 'چنګاښ',
    'asad': 'زمری',
    'sunbula': 'وږی',
    'mizan': 'تله',
    'aqrab': 'لړم',
    'qaws': 'لیندۍ',
    'jadi': 'مرغومی',
    'dalw': 'سلواغه',
    'hut': 'کب',
    // Weather
    'weather': 'هوا',
    'exchangeRate': 'د تبادلي نرخ',
    'newsTabsSports': 'ورزش',
    'newsTabsGeneral': 'عمومي',
    'newsTabsOthers': 'نور',
    'prayerTime': 'د لمونځ وخت',
    'dateConverter': 'د نېټې بدلوونکی',
    'dateConverterDescription': 'د میلادي، لمریز هجري او قمري نېټو ترمنځ بدلون.',
    'date': 'نېټه',
    'convert': 'بدلول',
    'miladi': 'میلادي',
    'gregorian': 'میلادي',
    'shamsi': 'لمریز هجري',
    'qamari': 'قمري',
    'prayerTimes': 'د لمانځه وختونه',
    'shamsiDate': 'لمریز هجري نېټه',
    'gregorianDate': 'میلادي نېټه',
    'selectShamsiDate': 'لمریز هجري نېټه وټاکئ',
    'showTimes': 'وختونه وښایئ',
    'validDateError': 'یوه سمه نېټه د YYYY/MM/DD په بڼه ولیکئ.',
    'validShamsiDateError': 'یوه سمه لمریز هجري نېټه د YYYY/MM/DD په بڼه ولیکئ.',
    'qamariConversionNote': 'قمري بدلون مدني جنتري کاروي؛ د سپوږمۍ رسمي لیدل کېدای شي یوه ورځ توپیر ولري.',
    'estimatedKabulTimes': 'د کابل اټکلي وختونه — د رسمي جدول لپاره د وختونو د جدول کرښې اضافه کړئ.',
    'fajr': 'فجر',
    'dhuhr': 'ظهر',
    'asr': 'عصر',
    'maghrib': 'ماښام',
    'isha': 'ماخستن',
    'dailyPrayerTimes': 'ورځني وختونه',
    'dateConverterShortDescription': 'لمریز هجري / میلادي / قمري',
    'currentPrayer': 'اوسنی لمونځ',
    'nextPrayer': 'راتلونکی لمونځ',
    'kabulTime': 'د کابل وخت',
    'currentWeather': 'اوسنۍ هوا',
    'temperature': 'تودوخه',
    'feelsLike': 'احساسیږي',
    'humidity': 'لندبل',
    'windSpeed': 'د باد سرعت',
    'pressure': 'د هوا فشار',
    'visibility': 'لیدل',
    'uvIndex': 'د UV شاخص',
    'airQuality': 'د هوا کیفیت',
    'weatherCondition': 'د هوا حالت',
    'sunrise': 'د لمر راختل',
    'sunset': 'د لمر پرېوتل',
    'locationPermissionDenied': 'د ځای د موندنې اجازه رد شوه',
    'weatherLoadError': 'د هوا د معلوماتو په بارولو کې ستونزه',
    'refreshWeather': 'د هوا تازه کول',
    'defaultLocation': 'کابل، افغانستان',
    'celsius': '°C',
    'fahrenheit': '°F',
    'kmh': 'کیلومتره په ساعت',
    'mph': 'مایل په ساعت',
    'hpa': 'هکتوپاسکال',
    'km': 'کیلومتره',
    'percent': '%',
  };

  // English Text
  static const Map<String, String> englishText = {
    'videos': 'Videos',
    'welcome': 'Welcome',
    'headerTitle': 'General Directorate of Administration',
    'aopWebsite': 'Office of the Prime Minister',
    'agencyList': 'Ministry List',
    'publicServices': 'Public Services',
    'passportServices': 'Passport Services',
    'changeLanguage': 'Change Language',
    'comments': 'Comments',
    'contactUs': 'Contact Us',
    'settings': 'Settings',
    'home': 'Home',
    'websiteNav': 'Website',
    'contactNav': 'Contact',
    'dayToDay': 'Day to Day',
    'dayToDayDescription': 'Daily utilities such as weather, exchange rate, news, Qibla, and prayer times will appear here soon.',
    'newsNav': 'News',
    'jobOpportunities': 'Job Opportunities',
    'searchJobs': 'Search job opportunities...',
    'noJobsFound': 'No job opportunities found',
    'noJobsFoundDescription': 'No job opportunities are currently available',
    'newJobs': 'New Positions',
    'expiringJobs': 'Expiring Positions',
    'expiredJobs': 'Expired Positions',
    'noNewJobsFound': 'No new positions found',
    'noNewJobsFoundDescription': 'No new positions are currently available',
    'noExpiringJobsFound': 'No expiring positions found',
    'noExpiringJobsFoundDescription': 'No positions are expiring soon',
    'noExpiredJobsFound': 'No expired positions found',
    'noExpiredJobsFoundDescription': 'No expired positions are available',
    // Qibla Compass
    'qiblaCompass': 'Qibla Compass',
    'qiblaDirection': 'Qibla Direction',
    'calibrateCompass': 'Calibrate Compass',
    'calibrationInstructions': 'For accurate calibration, move your device in a figure-eight motion',
    'locationPermissionRequired': 'Location permission is required',
    'enableLocationServices': 'Enable location services',
    'compassNotAvailable': 'Compass not available',
    'qiblaAngle': 'Qibla Angle',
    'yourLocation': 'Your Location',
    'distanceToKaaba': 'Distance to Kaaba',
    'kilometers': 'kilometers',
    'jobOpportunity': 'Job Opportunity',
    'jobInformation': 'Job Information',
    'jobDescription': 'Job Description',
    'noDescriptionAvailable': 'No description available',
    'jobType': 'Job Type',
    'status': 'Status',
    'announcementDate': 'Announcement Date',
    'deadline': 'Deadline',
    'contractDuration': 'Contract Duration',
    'applyNow': 'Apply Now',
    'applyViaEmail': 'Apply via Email',
    'applyOnline': 'Apply Online',
    'emailClientOpened': 'Email client opened successfully',
    'browserOpened': 'Browser opened successfully',
    'linkOpened': 'Link opened successfully',
    'emailClientError': 'Unable to open email client. Please check if you have an email app installed.',
    'browserError': 'Unable to open browser. Please check your internet connection.',
    'linkError': 'Unable to open the link.',
    'active': 'Active',
    'expired': 'Expired',
    'generalError': 'An error occurred',
    'noInternetConnection': 'No internet connection',
    'sharedFromApp': 'Shared from AOP App',
    'noResults': 'No results found',
    'feedbackGuide': 'Please share your feedback with us',
    'whatsapp': 'WhatsApp',
    'email': 'Email',
    'contactForm': 'Contact Form',
    'langGuide': 'Please select your language',
    'loading': 'Loading...',
    'changingLanguage': 'Changing language...',
    // News Screen Texts
    'newsTitle': 'News',
    'newsTabsAll': 'All',
    'newsTabsLatest': 'Latest News',
    'newsTabsSocial': 'Social',
    'newsTabsEconomic': 'Economic',
    'newsTabsPolitical': 'Political',
    'newsTabsCultural': 'Cultural',
    'searchNewsHint': 'Search news...',
    'clearSearch': 'Clear',
    'searchLabel': 'Searching for:',
    'emptySearchResult': 'No results found for "{query}"',
    'emptySearchSuggestion': 'Please search with different keywords',
    'clearSearchButton': 'Clear Search',
    'savedNews': 'Saved News',
    'noSavedNews': 'No saved news',
    'newsRemoved': 'News removed',
    'undo': 'Undo',
    'browseNews': 'Browse News',
    'newsSaved': 'News saved',
    'newsUnsaved': 'News unsaved',
    'clearAllSavedNews': 'Clear all saved news',
    'clearAllSavedNewsConfirm': 'Are you sure you want to clear all saved news?',
    'cancel': 'Cancel',
    'clear': 'Clear',
    'allNewsRemoved': 'All saved news removed',
    'save': 'Save',
    'changeTextSize': 'Change Text Size',
    'comingSoon': 'Coming Soon',
    'fullscreenComingSoon': 'Fullscreen view coming soon',
    'imageGallery': 'Image Gallery',
    'imageCount': '{count} Images',
    'noImages': 'No images available',
    'additionalInfo': 'Additional Information',
    'viewCount': '{count} Views',
    'noTitle': 'No Title',
    'viewFullNews': 'View full news in our app',
    'newsItem': 'News Item',
    'errorLoadingNews': 'Error loading news',
    // Ministry Screen Texts
    'ministries': 'Ministries',
    'searchMinistries': 'Search ministries...',
    'noMinistries': 'No ministries available',
    'ministryComingSoon': 'New ministries will be added soon',
    'ministryLoadError': 'Error loading ministry list',
    'tryAgain': 'Try Again',
    'viewDetails': 'View Details',
    'ministry': 'Ministry',
    'aboutMinistry': 'About the Ministry',
    'contactInfo': 'Contact Information',
    'viewOfficialWebsite': 'View Official Website',
    'phone': 'Phone',
    'website': 'Website',
    'share': 'Share',
    'shareError': 'Error sharing',
    'noWebsite': 'No website available',
    'back': 'Back',
    // Independent Directorates Text
    'independentDirectorates': 'Independent Directorates',
    'searchDirectorates': 'Search independent directorates...',
    'noDirectorates': 'No independent directorates available',
    'directorateComingSoon': 'New independent directorates will be added soon',
    'directorateLoadError': 'Error loading independent directorates list',
    'directorate': 'Independent Directorate',
    'aboutDirectorate': 'About the Independent Directorate',
    // Provinces Text
    'provinces': 'Provinces',
    'searchProvinces': 'Search provinces...',
    'noProvinces': 'No provinces available',
    'provinceComingSoon': 'New provinces will be added soon',
    'provinceLoadError': 'Error loading provinces list',
    'province': 'Province',
    'aboutProvince': 'About the Province',
    // Error messages
    'cannotOpenWebsite': 'Unfortunately, cannot open the website: {url}',
    'cannotMakeCall': 'Unfortunately, cannot make the call: {phone}',
    // API Error Messages
    'cannotLoadData': 'Failed to load data',
    // Filter related texts
    'filter': 'Filter',
    'filterByDepartment': 'Filter by Department',
    'allDepartments': 'All Departments',
    'activeJobs': 'Active Jobs',
    'clearFilter': 'Clear Filter',
    'applyFilter': 'Apply Filter',
    'noJobsInDepartment': 'No jobs found in this department',
    'noConnection': 'No internet connection',
    'timeout': 'Request timed out',
    'serverError': 'Server error occurred',
    'prayerTimesLoadError': 'Prayer times could not be loaded. Please try again.',
    'retry': 'Try Again',
    'backToHome': 'Back to Home',
    'offline': 'You are offline',
    'checkConnection': 'Please check your internet connection',
    // Contract Types
    'permanent': 'Permanent',
    'temporary': 'Temporary',
    'contract': 'Contract',
    'partTime': 'Part Time',
    'fullTime': 'Full Time',
    // Solar Hijri Months
    'hamal': 'Hamal',
    'sawr': 'Sawr',
    'jawza': 'Jawza',
    'saratan': 'Saratan',
    'asad': 'Asad',
    'sunbula': 'Sunbula',
    'mizan': 'Mizan',
    'aqrab': 'Aqrab',
    'qaws': 'Qaws',
    'jadi': 'Jadi',
    'dalw': 'Dalw',
    'hut': 'Hut',
    // Weather
    'weather': 'Weather',
    'exchangeRate': 'Exchange Rate',
    'newsTabsSports': 'Sports',
    'newsTabsGeneral': 'General',
    'newsTabsOthers': 'Others',
    'prayerTime': 'Prayer Time',
    'dateConverter': 'Date converter',
    'dateConverterDescription': 'Convert between Gregorian (Miladi), Solar Hijri (Shamsi), and Qamari dates.',
    'date': 'Date',
    'convert': 'Convert',
    'miladi': 'Miladi',
    'gregorian': 'Gregorian',
    'shamsi': 'Shamsi',
    'qamari': 'Qamari',
    'prayerTimes': 'Prayer times',
    'shamsiDate': 'Shamsi date',
    'gregorianDate': 'Gregorian date',
    'selectShamsiDate': 'Select Shamsi date',
    'showTimes': 'Show times',
    'validDateError': 'Enter a valid date as YYYY/MM/DD.',
    'validShamsiDateError': 'Use a valid Shamsi date: YYYY/MM/DD.',
    'qamariConversionNote': 'Qamari conversion uses the civil calendar; official moon sighting may differ by one day.',
    'estimatedKabulTimes': 'Estimated Kabul times — add the timetable rows to use official table values.',
    'fajr': 'Fajr',
    'dhuhr': 'Dhuhr',
    'asr': 'Asr',
    'maghrib': 'Maghrib',
    'isha': 'Isha',
    'dailyPrayerTimes': 'Daily times',
    'dateConverterShortDescription': 'Shamsi / Miladi / Qamari',
    'currentPrayer': 'Current prayer',
    'nextPrayer': 'Next prayer',
    'kabulTime': 'Kabul time',
    'currentWeather': 'Current Weather',
    'temperature': 'Temperature',
    'feelsLike': 'Feels Like',
    'humidity': 'Humidity',
    'windSpeed': 'Wind Speed',
    'pressure': 'Pressure',
    'visibility': 'Visibility',
    'uvIndex': 'UV Index',
    'airQuality': 'Air Quality',
    'weatherCondition': 'Weather Condition',
    'sunrise': 'Sunrise',
    'sunset': 'Sunset',
    'locationPermissionDenied': 'Location permission denied',
    'weatherLoadError': 'Error loading weather data',
    'refreshWeather': 'Refresh Weather',
    'defaultLocation': 'Kabul, Afghanistan',
    'celsius': '°C',
    'fahrenheit': '°F',
    'kmh': 'km/h',
    'mph': 'mph',
    'hpa': 'hPa',
    'km': 'km',
    'percent': '%',
  };
}
